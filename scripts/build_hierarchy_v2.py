"""
Build a hierarchy.json from the freshly-pulled EEE datastore, applying:
  - family-stem grouping (spec §1–§7)
  - cross-suite benchmark_index (spec §8)
  - raw_verified-driven first/third-party rule (overrides existing labels)
  - canonical IDs/aliases from evaleval/entity-registry-data
  - alphaxiv folder excluded (bad data)

Inputs:
  .cache/eee_datastore/data/<folder>/<org>/<model>/<id>.json   (EEE records)
  .cache/entity_registry/{aliases,canonical_benchmarks,...}    (canonical IDs)

Outputs:
  output/hierarchy_v2.json
  output/hierarchy_v2_explorer.html
"""
...
EEE_DIR = ROOT / ".cache/eee_datastore/data"
REGISTRY_DIR = ROOT / ".cache/entity_registry"
# Local taxonomy override — same YAMLs the production pipeline picks up via
# EVALCARD_REGISTRY_SEED_DIR. Keeping the curation here means the local v2
# build and the warehouse pipeline read from one source of truth without
# touching the upstream registry repo.
OVERRIDE_DIR = ROOT / "temp_registry_override"
OUT_DIR = ROOT / "output"
OUT_JSON = OUT_DIR / "hierarchy_v2.json"
OUT_HTML = OUT_DIR / "hierarchy_v2_explorer.html"
...
def _load_override_yaml() -> dict[str, Any]:
    """Read families.yaml / composites.yaml / slice_overrides.yaml /
    display_overrides.yaml from temp_registry_override/. Returns a dict
    {composites, families, slice_promotions, acronyms} with safe defaults
    when files are missing — falls back to in-code constants below."""
    out: dict[str, Any] = {
        "composites": {},
        "families": {},
        "slice_promotions": set(),
        "acronyms": set(),
        "family_primary_override": {},
    }
    if not OVERRIDE_DIR.exists():
        print(f"[override] {OVERRIDE_DIR} missing, using in-code defaults",
              file=sys.stderr)
        return out
    try:
        import yaml as _yaml
    except ImportError:
        print("[override] PyYAML not installed; using in-code defaults",
              file=sys.stderr)
        return out
    ...
    def _read(path: Path) -> Any:
        ...
        try:
            ...
            print(f"[override] failed to read {path.name}: {exc}", file=sys.stderr)
            return None

    composites = _read(OVERRIDE_DIR / "composites.yaml") or {}
    families = _read(OVERRIDE_DIR / "families.yaml") or {}
    slice_overrides = _read(OVERRIDE_DIR / "slice_overrides.yaml") or {}
    display_overrides = _read(OVERRIDE_DIR / "display_overrides.yaml") or {}

    out["composites"] = composites if isinstance(composites, dict) else {}
    out["families"] = families if isinstance(families, dict) else {}
    if isinstance(slice_overrides, dict):
        prom = slice_overrides.get("promote_to_benchmark") or []
        if isinstance(prom, list):
            out["slice_promotions"] = {str(b) for b in prom}
        primary = slice_overrides.get("family_primary_override") or {}
        if isinstance(primary, dict):
            out["family_primary_override"] = {str(k): str(v) for k, v in primary.items()}
    if isinstance(display_overrides, dict):
        ac = display_overrides.get("acronyms") or []
        if isinstance(ac, list):
            out["acronyms"] = {str(s).lower() for s in ac if s}

    print(f"[override] loaded {len(out['composites'])} composites, "
          f"{len(out['families'])} families, "
          f"{len(out['slice_promotions'])} slice promotions, "
          f"{len(out['acronyms'])} acronyms",
          file=sys.stderr)
    return out


_OVERRIDE = _load_override_yaml()


def _load_categorized_raw() -> dict:
    """Read categorized.json verbatim — slugification happens lazily because
    `slugify` isn't defined yet at module-load time."""
    ...
    try:
        ...
        print(f"[override] failed to read categorized.json: {exc}", file=sys.stderr)
        return {}
...
# Build folder → family-stem map from composites.yaml + families.yaml.
# composites.yaml: composite_slug → {display, configs}.
# families.yaml:   family_id → {display, benchmarks: [composite_slug, ...]}.
# So folder (config) → composite_slug (member of families) → family_id.
def _build_folder_to_family() -> tuple[dict[str, str], dict[str, str], dict[str, str]]:
    """Returns (folder_to_family, folder_to_composite, family_displays)."""
    folder_to_family: dict[str, str] = {}
    folder_to_composite: dict[str, str] = {}
    family_displays: dict[str, str] = {}

    composite_to_family: dict[str, str] = {}
    for fid, entry in _OVERRIDE["families"].items():
        if not isinstance(entry, dict):
            continue
        family_displays[fid] = entry.get("display") or fid
        for member in entry.get("benchmarks") or []:
            composite_to_family[member] = fid

    for slug, entry in _OVERRIDE["composites"].items():
        if not isinstance(entry, dict):
            continue
        configs = entry.get("configs") or [slug]
        family_id = composite_to_family.get(slug)  # None if singleton
        for cfg in configs:
            folder_to_composite[cfg] = slug
            if family_id:
                folder_to_family[cfg] = family_id
            else:
                # Singleton: composite slug doubles as family id
                folder_to_family[cfg] = slug
                family_displays.setdefault(slug, entry.get("display") or slug)
    return folder_to_family, folder_to_composite, family_displays


_FOLDER_TO_FAMILY, _FOLDER_TO_COMPOSITE, _FAMILY_DISPLAYS = _build_folder_to_family()
...
ACRONYMS |= _OVERRIDE.get("acronyms") or set()
...
# Merge in YAML-supplied primaries (slice_overrides.yaml: family_primary_override)
FAMILY_PRIMARY_OVERRIDE.update(_OVERRIDE.get("family_primary_override") or {})
...
def family_stem(folder: str, reg: dict[str, Any]) -> str:
    # 0. YAML override — temp_registry_override/families.yaml + composites.yaml
    #    is the curator-supplied source of truth. Wins over the in-code map
    #    so fixes can ship without a code change.
    if folder in _FOLDER_TO_FAMILY:
        return _FOLDER_TO_FAMILY[folder]
    # 1. Explicit in-code alias map (legacy fallback)
...
def build() -> dict:
    ...
    for folder, rec in iter_eee_records(EEE_DIR):
        ...
        # YAML override first; fall back to the legacy in-code composite map.
        # A composite layer is only emitted when the folder belongs to a
        # multi-composite family (HELM, HAL, tau-bench, etc.); singleton
        # families collapse the composite into the family directly.
        composite = _FOLDER_TO_COMPOSITE.get(folder)
        if composite and folder in _FOLDER_TO_FAMILY and _FOLDER_TO_FAMILY[folder] == composite:
            # Singleton — composite_slug == family_id; no extra layer needed
            composite = None
