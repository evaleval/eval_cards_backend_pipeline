...
_FALLBACK_RULES: tuple[tuple[re.Pattern[str], list[str]], ...] = (
    (re.compile(r"\b(?:safety|harmful|toxic|truthful|unsafe|civilcomments|jailbreak|red[\-_]?team|adversarial)\b", re.I),
     ["safety"]),
    (re.compile(r"\b(?:agent|swe[\-_]?bench|terminal[\-_]?bench|tau[\-_]?bench|appworld|browsecomp)", re.I),
     ["agentic"]),
    ...
    (re.compile(r"\b(?:multimodal|vision|vqa|mmmu|image|video|visual)\b", re.I),
     ["multimodal"]),
    (re.compile(r"\b(?:hallucin|faithful|factual)", re.I),
     ["hallucination"]),
    (re.compile(r"\b(?:robust|perturbation|noisy|corrupt)", re.I),
     ["robustness"]),
